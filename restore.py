import json
with open('d:\\waterapp\\app\\invoices\\InvoiceFormModal.tsx', 'w', encoding='utf-8') as f:
    f.write("""\"use client\";
    ...
    const modalContent = (
        <div className="warm-portal-overlay-fullscreen" onClick={onClose}>
            ...
            <div className="cinematic-scroll glass-modal-container" onClick={(e) => e.stopPropagation()} style={{ 
                width: '950px', maxHeight: '95vh', background: 'rgba(248, 250, 252, 0.85)', 
                backdropFilter: 'blur(25px)', borderRadius: '25px', padding: '15px 25px', 
                ...
                <div style={{ background: 'rgba(255,255,255,0.4)', padding: '10px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.7)', marginBottom: '10px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 0.5fr', gap: '10px', alignItems: 'end' }}>
                        <div>
                            <label style={{ fontSize: '12px', fontWeight: 900, color: THEME.primary, display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '6px' }}>البيان التفصيلي</label>
                            <input type="text" value={record.description ?? ''} onChange={(e) => setRecord({...record, description: e.target.value})} className="glass-input-field" />
                        </div>
                ...
                <div className="responsive-summary-grid" style={{ marginTop: '15px', padding: '15px', background: 'linear-gradient(135deg, #1e293b, #0f172a)', borderRadius: '24px', color: 'white', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.05)', padding: '10px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)' }}>
                        <div style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 800 }}>الضريبة (15%)</div>

...
export default function ExpensesPage() {
  ...
  useEffect(() => {
      setMounted(true);
      if (searchParams.get('from_inv') === '1') {
          const partner = searchParams.get('partner') || '';
          const item = searchParams.get('item') || '';
          const qty = searchParams.get('qty') || '0';
          
          logic.setCurrentExpense({
            creditor_account: partner,
            payee_name: partner,
  ...
  const expenseColumns = [
    ...
    { 
      header: renderSortableHeader('رقم الإذن', 'expense_number'), 
      accessor: 'expense_number', 
      render: (row: any) => row ? <span style={{ color: THEME.primary, fontWeight: 900 }}>{row.expense_number || '---'}</span> : null,
      exportValue: (row: any) => row.expense_number || '---' 
    },
    { 
    ...
    { 
      header: renderSortableHeader('المستفيد', 'payee_name'), 
      accessor: 'payee_name', 
      render: (row: any) => row ? (
            ...
            {needsPayment && (
             <button 
                  ...
                  onClick={(e) => {
                      ...
                      let resolvedDebitName = '';
                      const targetSupplierAccount = row.payment_account || row.creditor_account;
                      if (logic.accounts_raw && targetSupplierAccount) {
                          let foundAcc = logic.accounts_raw.find((a: any) => 
                              `${a.code} - ${a.name}` === targetSupplierAccount || a.name === targetSupplierAccount
                          );
                          if (!foundAcc) {
                              const match = targetSupplierAccount.match(/^(\d+)\s*-/);
                              if (match && match[1]) {
                                  foundAcc = logic.accounts_raw.find((a: any) => a.code === match[1]);
                              }
                          }
                          if (!foundAcc) {
                              foundAcc = logic.accounts_raw.find((a: any) => 
                                  a.name.includes(targetSupplierAccount.replace(/^\d+\s*-\s*/, '')) || 
                                  targetSupplierAccount.includes(a.name)
                              );
                          }
                          
                          if (foundAcc) {
                              resolvedDebitId = foundAcc.id;
                              resolvedDebitName = foundAcc.name;
                          }
                      }
      ...
      {logic.selectedIds.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '5px', paddingTop: '15px', borderTop: '1px dashed rgba(255,255,255,0.2)' }}>
          <div style={{ textAlign: 'center', marginBottom: '5px' }}>
              <p style={{ fontSize: '11px', color: '#475569', fontWeight: 900, margin: 0 }}>الإجراءات على ({logic.selectedIds.length})</p>
  ...
  return (
    <div className="clean-page">
      <MasterPage icon="💸" title="سجل المصروفات الموحد" subtitle="إدارة التكاليف والمشتريات وتوزيع الأصناف">
            ...
            <div style={{ position: 'fixed', inset: 0, zIndex: 999999999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', background: 'rgba(40, 24, 10, 0.85)', backdropFilter: 'blur(10px)', padding: '50px 20px', overflowY: 'auto' }}>
              ...
              <div className="cinematic-scroll" style={{ background: 'rgba(255, 255, 255, 0.95)', borderRadius: '32px', width: '100%', maxWidth: '600px', padding: '40px', position: 'relative', zIndex: 10, margin: 'auto', boxShadow: '0 50px 100px -20px rgba(0,0,0,0.5)', animation: 'modalEntrance 0.4s forwards' }}>
                ...
                <div style={{ display: 'flex', gap: '15px', marginTop: '40px' }}>
                  ...
                  <button onClick={()=>logic.setIsBulkFixModalOpen(false)} style={{ flex: 1, padding: '18px', borderRadius: '16px', border: '2px solid rgba(40, 145, 200, 0.15)', background: 'white', color: '#64748b', fontWeight: 900, cursor: 'pointer', fontSize: '16px' }}>إلغاء</button>
                </div>
              </div>
            </div>,
            document.body

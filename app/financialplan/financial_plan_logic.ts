...
const DEFAULT_ITEMS = [
    { category: 'إيرادات', item_name: 'مبيعات الفروع' },
...
export function useFinancialPlanLogic() {
    ...
    const fetchPlan = async () => {
        ...
        try {
            ...
            // لو مفيش داتا للشهر ده، هنفرش الأصناف الافتراضية
    ...
    const savePlanToDB = async () => {
        ...
        try {
            ...
            const recordsToSave = liveRecords.map(r => ({
                ...
                category: r.category,
                item_name: r.item_name || 'صنف غير مسمى',
            ...
            showGlobalToast("✅ تم حفظ وتثبيت الخطه المالية بنجاح تام!", 'warning');
            await fetchPlan(); // تحديث فوري للشاشة لجلب البيانات المستقرة من الداتابيز
        } catch (err: any) {
            showGlobalToast(`❌ خطأ في الحفظ: ${err.message}`, 'warning');

...
export function usePartnersLogic() {
  ...
  const handleSavePartner = async () => {
    if (!newPartner.name || !newPartner.code) {
      return showGlobalToast('الرجاء إدخال الاسم والكود!', 'warning');
    }
    ...
    if (error) {
      showGlobalToast('خطأ في العملية: ' + error.message, 'warning');
    } else {
      showGlobalToast(editingId ? 'تم تحديث البيانات بنجاح! ✅' : 'تم إضافة الشريك بنجاح! 🎉', 'warning');
      closeModal();
      fetchPartners();
    }
    setIsSaving(false);
  ...
  const handleDelete = async (id: string) => {
    if (confirm('هل أنت متأكد من حذف هذا الكيان نهائياً من سجلات مياه غيام؟')) {
  ...
  const stats = {
    ...
    contractors: partners.filter(p => p.type === 'مورد').length
  };
  ...
  const getTypeStyle = (type: string) => {
    switch(type) {
      ...
      case 'مورد': return { bg: '#FEF3C7', color: '#B45309', icon: '🏗️' };

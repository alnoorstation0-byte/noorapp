...
const TABLE_GROUPS = [
  ...
  {
    name: "🏗️ الفروع والعمليات الميدانية",
    tables: [
  ...
  },
  {
    ...
    tables: [
      ...
      { id: 'material_receipts', name: 'سندات استلام المخزون' },
      { id: 'material_receipt_lines', name: 'تفاصيل استلام المخزون' },
      { id: 'material_issues', name: 'سندات صرف المخزون' },
      { id: 'material_issue_lines', name: 'تفاصيل صرف المخزون' },
    ]
  },
  {
...
export default function SettingsPage() {
  ...
  return (
    <MasterPage icon="⚙️" title="إعدادات النظام والنسخ الاحتياطي" hideTitleOnMobile={true}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', animation: 'fadeUp 0.5s ease-out' }}>
        ...
        {activeTab === 'backup' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 2fr) minmax(250px, 1fr)', gap: '30px', alignItems: 'start' }}>
            ...
            <GlassContainer>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', borderBottom: '2px solid rgba(255, 255, 255, 0.4)', paddingBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                ...
                <button onClick={selectAllTables} style={{ background: 'rgba(255, 255, 255, 0.6)', border: '1px solid rgba(40, 145, 200, 0.15)', padding: '8px 16px', borderRadius: '10px', cursor: 'pointer', fontWeight: 900, color: THEME.primary, transition: '0.2s', fontSize: '12px' }}>
                ...
                {isUploading ? (
                    ...
                    <>
                        ...
                        <div>
                            ...
                            {!selectedFile && <div style={{ fontSize: '11px', color: '#475569', fontWeight: 700 }}>.xlsx فقط</div>}
                        </div>
                    </>
                )}
                <input 
      ...
      <style>{`
        .tab-btn { padding: 12px 24px; border-radius: 12px; border: 1px solid rgba(40, 145, 200, 0.15); font-weight: 900; font-size: 13px; cursor: pointer; transition: 0.3s; background: white; color: #64748b; white-space: nowrap; }
        .tab-btn:hover { background: rgba(255, 255, 255, 0.6); border-color: rgba(40, 145, 200, 0.2); }
        .tab-btn.active { background: #0f172a; color: white; border-color: #0f172a; box-shadow: 0 4px 15px rgba(15, 23, 42, 0.2); }
        .group-card { background: rgba(255, 255, 255, 0.6); padding: 15px; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.4); transition: 0.2s; }
        .table-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: white; border-radius: 8px; margin-top: 5px; border: 1px solid rgba(40, 145, 200, 0.15); transition: 0.2s; }
        .table-row.selected { border-color: #0ea5e9; background: #f0f9ff; }
        .table-row:hover { border-color: rgba(40, 145, 200, 0.2); }
        ...
        .premium-dropzone { background: rgba(255, 255, 255, 0.6); border: 2px dashed rgba(40, 145, 200, 0.2); border-radius: 16px; padding: 30px 20px; text-align: center; cursor: pointer; transition: all 0.3s ease; display: flex; flex-direction: column; gap: 10px; justify-content: center; align-items: center; }
        .premium-dropzone:hover { background: white; border-color: #475569; }
        ...
        .sidebar-summary-glass { background: rgba(255,255,255,0.8); padding: 20px; border-radius: 16px; text-align: center; border: 1px solid rgba(40, 145, 200, 0.15); }

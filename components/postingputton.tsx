    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
            {kpis.map((kpi, idx) => (
              <div key={idx} style={{ background: 'rgba(255, 255, 255, 0.6)', padding: '15px', borderRadius: '16px', border: `1px solid ${kpi.color}20`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <input 
              ...
              style={{ padding: '12px', borderRadius: '12px', border: `1.5px solid rgba(40, 145, 200, 0.15)`, outline: 'none', fontSize: '14px' }} 
            />
            {filtersSlot}
        </div>
            ...
            <div style={{ 
              ...
              border: '1px solid rgba(40, 145, 200, 0.15)'
                ...
                {isSingleSelection && onEdit && (
                    <button onClick={() => { onEdit(); setIsOpen(false); }} style={{ ...btnStyle, background: 'rgba(40, 145, 200, 0.15)', color: THEME.primary }}>
                      📝 تعديل البيانات
                    </button>
                )}
...
export function PaginationPanel({ totalItems, currentPage, rowsPerPage, onPageChange, onRowsChange }: any) {
  ...
  const btnActionStyle = { padding: '8px 16px', borderRadius: '8px', border: '1px solid rgba(40, 145, 200, 0.15)', background: 'white', cursor: 'pointer', fontSize: '13px', fontWeight: 600 };

  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '15px 25px', borderRadius: '16px', marginTop: '20px', direction: 'rtl', boxShadow: '0 4px 12px rgba(0,0,0,0.03)', border: '1px solid rgba(255, 255, 255, 0.4)' }}>
      <div style={{ fontSize: '14px', color: '#64748b' }}>
        <span>عرض: </span>
        <select 
          value={rowsPerPage} 
          onChange={(e) => onRowsChange(Number(e.target.value))}
          style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid rgba(40, 145, 200, 0.15)', margin: '0 5px', outline: 'none' }}
        >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          ...
          <span style={{ color: 'rgba(40, 145, 200, 0.2)' }}>/</span>

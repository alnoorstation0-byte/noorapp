...
export default function InvoiceAgingDashboard({ summary }: { summary: any }) {
  const aging = summary?.aging;
  // لو مفيش ديون خالص، مش هنعرض حاجة
  if (!aging || summary.totalRemaining === 0) return null;

  // دالة لحساب النسبة المئوية لكل فئة عشان نظبط عرض الشريط
  const getWidth = (value: number) => `${(value / summary.totalRemaining) * 100}%`;

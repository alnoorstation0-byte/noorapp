...
export default function ProjectBoqCombo({ projectId, value, initialDisplay, onSelect, placeholder = "اختر البند / أمر الشغل..." }: any) {
    ...
    return (
        <div ref={dropdownRef} style={{ position: 'relative', width: '100%' }}>
            ...
            >
                <span style={{ fontWeight: 800, color: displayVal ? '#1e293b' : '#475569', fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {isLoading ? '⏳ جاري جلب أوامر الشغل...' : (!projectId ? '⚠️ اختر العقار أولاً' : (displayVal || placeholder))}
                </span>
                <span style={{ color: '#475569', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: '0.2s', fontSize: '10px' }}>▼</span>
            </div>
            
            {isOpen && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: `1px solid ${THEME.sandDark || 'rgba(40, 145, 200, 0.15)'}`, borderRadius: '12px', marginTop: '8px', zIndex: 9999, boxShadow: '0 10px 30px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
                    <div style={{ padding: '8px', background: 'rgba(255, 255, 255, 0.6)', borderBottom: '1px solid rgba(40, 145, 200, 0.15)' }}>
                        <input 
                            type="text" 
                            ...
                            style={{ width: '100%', padding: '8px 12px', border: '1px solid rgba(40, 145, 200, 0.2)', borderRadius: '6px', outline: 'none', fontSize: '12px', fontWeight: 700, color: '#1e293b' }}
                        />
                    </div>
                    <div className="custom-scrollbar" style={{ maxHeight: '200px', overflowY: 'auto', padding: '5px' }}>
                        {filteredOptions.length > 0 ? filteredOptions.map((opt: any) => (
                            <div 
                                ...
                                style={{ padding: '10px', cursor: 'pointer', borderRadius: '6px', fontSize: '12px', fontWeight: 800, color: value === (opt.boq_item_id || opt.id) ? THEME.primary : '#334155', background: value === (opt.boq_item_id || opt.id) ? '#eff6ff' : 'transparent', borderBottom: '1px solid rgba(255, 255, 255, 0.4)', transition: '0.2s' }}
                            ...
                            <div style={{ padding: '15px', textAlign: 'center', fontSize: '12px', color: '#475569', fontWeight: 700 }}>لا توجد بنود/أوامر شغل لهذا العقار</div>
                        )}
                    </div>
                </div>
            )}

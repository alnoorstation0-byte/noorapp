...
export default function SignaturePad({ userId, currentSignature, onSaved }: { userId: string, currentSignature?: string, onSaved?: () => void }) {
  ...
  return (
    <div style={{ background: 'rgba(255, 255, 255, 0.6)', padding: '20px', borderRadius: '15px', border: `1px solid ${THEME.border}`, maxWidth: '500px', margin: '0 auto', width: '100%' }}>
      ...
      {currentSignature && (
        <div style={{ marginBottom: '15px', textAlign: 'center' }}>
          ...
          <img src={currentSignature} alt="Current Signature" style={{ maxHeight: '60px', background: 'white', padding: '5px', borderRadius: '8px', border: '1px dashed rgba(40, 145, 200, 0.2)' }} />
        </div>
      )}

      {/* الـ Canvas اللي بيترسم فيه */}
      <div style={{ border: '2px dashed rgba(40, 145, 200, 0.2)', borderRadius: '10px', background: 'white', overflow: 'hidden', marginBottom: '15px', display: 'flex', justifyContent: 'center' }}>
        <SignatureCanvas 
          ref={sigCanvas}
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        ...
        <button onClick={clear} style={{ flex: 1, background: 'rgba(40, 145, 200, 0.15)', color: '#475569', padding: '10px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>
          🧹 مسح
        </button>
      </div>
    </div>
